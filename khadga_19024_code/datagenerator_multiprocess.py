...
def augment_image(x):
    ...
    """
    image_dir:str = x.image_dir
    classes:str = x.classes
    out_dir:str = x.out_dir
    train_size:int = x.train_size
    test_size:int = x.test_size 
...
if __name__ == '__main__':
    ...
    pandarallel.initialize(progress_bar=0,nb_workers=10)
    
    df = pd.DataFrame({'image_dir': image_dir, 
                       'classes' : os.listdir(image_dir),
                       'out_dir': out_dir,
                       'train_size': train_size,
                       'test_size':  test_size                     
                       })
    df.parallel_apply(augment_image,axis = 1)
